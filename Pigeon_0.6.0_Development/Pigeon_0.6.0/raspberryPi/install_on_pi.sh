...
INSTALL_DIR="${PIGEON_INSTALL_DIR:-${INSTALL_HOME}/Pigeon_0.6.0}"
IN_PLACE=0
ENABLE_AUTOSTART=1
MAKE_SHORTCUTS=1
SKIP_APT=0
...
if ! sudo -u "${INSTALL_USER}" bash -c "cd '${INSTALL_DIR}' && ./run_pigeon_0_6.sh --bootstrap-only"; then
...
echo "  cd '${INSTALL_DIR}' && ./run_pigeon_0_6.sh"
echo ""
if [[ "${ENABLE_AUTOSTART}" -eq 1 ]] && command -v systemctl >/dev/null 2>&1; then
