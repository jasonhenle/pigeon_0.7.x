...
_UA = "Pigeon/0.6 (github-update)"
_LAUNCHER_NAMES = ("run_pigeon_0_6.sh", "Run-Pigeon", "run-pigeon.sh")
...
def resolve_install_root(*, script_path: str | Path | None = None) -> Path | None:
    """Return the Pigeon app folder (contains ``run_pigeon_0_6.sh`` and ``pigeonSystem/``)."""
    ...
    for base in candidates:
        ...
        if key in seen:
            continue
        seen.add(key)
        if (b / "pigeonSystem" / "pigeon_0_6.py").is_file() and any(
...
def _find_app_root_in_tree(root: Path) -> Path | None:
    if (root / "pigeonSystem" / "pigeon_0_6.py").is_file() and any(
        (root / name).is_file() for name in _LAUNCHER_NAMES
    ):
        return root
    try:
        for launcher in root.rglob("run_pigeon_0_6.sh"):
            parent = launcher.parent
            if (parent / "pigeonSystem" / "pigeon_0_6.py").is_file():
                return parent
    except OSError:
        pass
    return None
    ...
    try:
        ...
        if app_src is None:
            return ApplyUpdateResult(
                False,
                "Could not find Pigeon app folder (run_pigeon_0_6.sh) inside GitHub zip.",
            )
