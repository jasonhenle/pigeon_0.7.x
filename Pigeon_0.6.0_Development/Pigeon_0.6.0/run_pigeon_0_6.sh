#!/bin/bash
# Pigeon 0.6 launcher for Linux / Raspberry Pi OS.
set -euo pipefail
...
MAIN_PY="${SYSTEM_DIR}/pigeon_0_6.py"
...
if [[ ! -f "${STATE_DIR}/tmdb_api_key" && ! -f "${STATE_DIR}/tmdb_read_token" ]]; then
  if [[ -f "${SCRIPT_DIR}/setup/tmdb_api_key" ]]; then
    cp "${SCRIPT_DIR}/setup/tmdb_api_key" "${STATE_DIR}/tmdb_api_key"
    chmod 600 "${STATE_DIR}/tmdb_api_key" 2>/dev/null || true
  elif [[ -f "${SCRIPT_DIR}/setup/tmdb_read_token" ]]; then
    cp "${SCRIPT_DIR}/setup/tmdb_read_token" "${STATE_DIR}/tmdb_read_token"
