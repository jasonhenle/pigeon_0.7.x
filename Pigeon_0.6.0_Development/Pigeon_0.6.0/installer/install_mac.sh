...
INSTALL_DIR="${PIGEON_INSTALL_DIR:-${HOME}/Applications/Pigeon_0.6.0}"
IN_PLACE=0
MAKE_SHORTCUT=1
...
chmod +x "${INSTALL_DIR}/run_pigeon_0_6.command" "${INSTALL_DIR}/run_pigeon_0_6.sh" 2>/dev/null || true
...
bash "${INSTALL_DIR}/run_pigeon_0_6.command" --help >/dev/null

if [[ "${MAKE_SHORTCUT}" -eq 1 ]]; then
  SHORTCUT="${HOME}/Desktop/Pigeon.command"
  cat > "${SHORTCUT}" <<EOF
#!/bin/bash
exec "${INSTALL_DIR}/run_pigeon_0_6.command"
EOF
  chmod +x "${SHORTCUT}"
...
echo "  Launch:         ${INSTALL_DIR}/run_pigeon_0_6.command"
if [[ "${MAKE_SHORTCUT}" -eq 1 ]]; then
