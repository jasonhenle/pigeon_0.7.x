...
def pigeon_python_dir() -> Path:
    """Directory that should contain pigeon_0_5.py (…/Pigeon_python)."""
    return _pigeon_package_dir().parent
...
_LOGO_EXACT_NAMES = (
    "P_0.5_pigeonLogo_4x6_MEDIUM_pigeonLogo.png",
    "P_0.5_pigeonLogo_4x6_MEDIUM_pigeonLogo.PNG",
    "Pigeon_Logo.png",
    "pigeon_logo.png",
    "PigeonLogo.png",
...
_LOGO_EXACT_LOWER = frozenset(n.lower() for n in _LOGO_EXACT_NAMES)
_LOGO_RASTER_SUFFIXES = frozenset({".png", ".jpg", ".jpeg", ".webp"})


def _is_pigeon_logo_candidate(p: Path) -> bool:
    if not p.is_file() or p.suffix.lower() not in _LOGO_RASTER_SUFFIXES:
        return False
    nl = p.name.lower()
    if nl in _LOGO_EXACT_LOWER:
        return True
    # CamelCase "pigeonLogo" lowercases to "pigeonlogo" — still matches both tokens:
    if "pigeon" in nl and "logo" in nl:
        return True
    return "pigeonlogo" in nl or "p_0.5_pigeonlogo" in nl


def _collect_logo_pngs(root: Path, *, max_depth: int) -> list[Path]:
    out: list[Path] = []
    ...
    def walk(d: Path, depth: int) -> None:
        ...
        try:
            for p in d.iterdir():
                if _is_pigeon_logo_candidate(p):
                    out.append(p)
                elif p.is_dir():
                    walk(p, depth + 1)
        except OSError:
...
def _logo_path_score(p: Path) -> int:
    nl = p.name.lower()
    blob = f"{p.parent.name}/{p.name}".lower()
    if "pigeonassets" in blob.replace("_", ""):
        bonus = 15
    else:
        bonus = 0
    if "p_0.5_pigeonlogo" in nl or "p_0.5_pigeon_logo" in nl:
        return 120 + bonus
    if nl == "pigeon_logo.png":
        return 100 + bonus
    if nl in _LOGO_EXACT_LOWER:
        return 90 + bonus
    if "pigeon" in nl and "logo" in nl:
...
def pick_pigeon_logo_png(poster_widget_dir: Path | None = None) -> Path | None:
    ...
    if poster_widget_dir is not None and poster_widget_dir.is_dir():
        for name in _LOGO_EXACT_NAMES:
            c = poster_widget_dir / name
            if c.is_file():
                return c
        found.extend(_collect_logo_pngs(poster_widget_dir, max_depth=4))
    found = _dedupe(found)
    if found:
        found.sort(
            key=lambda p: (-_logo_path_score(p), -_video_mtime(p), len(str(p)), str(p).lower())
        )
        return found[0]
    ...
    for assets_root in pigeon_logo_asset_dir_candidates():
        for name in _LOGO_EXACT_NAMES:
            ...
            if c.is_file():
                return c
        found.extend(_collect_logo_pngs(assets_root, max_depth=8))
    ...
    for bundle in _all_pigeon05_roots():
        if bundle.is_dir():
            found.extend(_collect_logo_pngs(bundle, max_depth=14))
    found = _dedupe(found)
    if not found:
        return None
    found.sort(
